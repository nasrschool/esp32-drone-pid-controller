...
#define ESC_PIN (32)  // connected to ESC control wire M1 brown
#define ESC_PIN2 (33)
#define ESC_PIN3 (25)
#define ESC_PIN4 (26)
...
void setup() {
  ...
  myESC2.setPeriodHertz(50);
  myESC3.setPeriodHertz(50);
  myESC4.setPeriodHertz(50);
  */
  

  delay(1000);
  /*
  ledcAttachChannel(ESC_PIN,  freq, resolution, CH_M1);
  ledcAttachChannel(ESC_PIN2, freq, resolution, CH_M2);
  ledcAttachChannel(ESC_PIN3, freq, resolution, CH_M3);
  ledcAttachChannel(ESC_PIN4, freq, resolution, CH_M4);
  */
  delay(1000);
  ...
  Serial.println("start arming: ");
   for(int i=0;i<1000;i++) {
    ...
    //writeMicroseconds(CH_M1, 1000);
    //writeMicroseconds(CH_M2, 1000);
    //writeMicroseconds(CH_M3, 1000);
    //writeMicroseconds(CH_M4, 1000);
...
void writeMicroseconds(int channel, int us) {
  // Duty cycle calculation: (Pulse Width / Period) * (2^Resolution - 1)
  uint32_t duty = (uint32_t)(((uint64_t)us * 65535) / periodUs);
...
void loop() {
  ...
  if (currMillis - prevMillis >= 4) {
    ...
    base = thrust;
    degreeInRadian = (kalmanPitch * PI) / 180;

    M1 = 1100 + ((base - 1100) / cos(degreeInRadian)) + (PIDp / 4);
    M2 = 1100 + ((base - 1100) / cos(degreeInRadian)) + (PIDp / 4);
    M3 = 1100 + ((base - 1100) / cos(degreeInRadian)) - (PIDp / 4);
    M4 = 1100 + ((base - 1100) / cos(degreeInRadian)) - (PIDp / 4);
    ...
    if (kalmanPitch > 45 || kalmanPitch < -45) {
      M1 = 1100;
      M2 = 1100;
      M3 = 1100;
      M4 = 1100;
    ...
    //32 33 left
    // the code REQUIRED to give commands to the motor introduces a 40ms delay
    // how in the world bruh
    // debugging this took SO LONG
    myESC.speed(M1);
    myESC2.speed(M2);
    myESC3.speed(M3);
    myESC4.speed(M4);
    ...
    /*
    writeMicroseconds(CH_M1, (int)M1);
    writeMicroseconds(CH_M2, (int)M2);
    writeMicroseconds(CH_M3, (int)M3);
    writeMicroseconds(CH_M4, (int)M4);
    */
    /*7
